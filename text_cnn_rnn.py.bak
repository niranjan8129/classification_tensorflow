...
class TextCNNRNN(object):
	def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
		...
		lstm_cell = lstm_cell.get_dropout_mask_for_cell(training=True,count=10 )
		...
		#outputs, state = tf.compat.v1.nn.rnn_cell(lstm_cell, inputs, initial_state=self._initial_state, sequence_length=self.real_len)
		#outputs, state = tf.compat.v1.nn.static_rnn(lstm_cell, inputs, initial_state=self._initial_state)
		...
		#outputs, state = tf.compat.v1.nn.static_rnn(lstm_cell, inputs, initial_state=self._initial_state,return_sequences=True, return_state=True)
